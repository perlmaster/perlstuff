...
sub list_compact
{
	...
	if ( @list > 0 ) {
		...
		@list = @list[@indexes];
		list_columns_style_unsorted(\@list,$line_limit,undef,\*STDOUT);
		print "\n";
	} # IF
	else {
		print "** empty list **\n";
...
MAIN:
{
	...
	my ( $elapsed_time , @file_times , @file_sizes );
	...
	$status = getopts("hkDdfFtTspScmy:e:riMaPR",\%options);
	if ( $options{"h"} ) {
		display_pod_help($0);
		exit 0;
	} # IF
	...
	if ( $count == 0 ) {
		...
		if ( $options{"P"} ) {
			...
			foreach my $entry ( @ARGV ) {
				if ( -d $entry && $options{"D"} == 0 ) {
					$status = get_dir_entries($entry,{ 'dot' => $options{'a'} , 'qual' => 1 , 'sort' => 0 },\@entries,\$errmsg);
					if ( $status < 0 ) {
						die("$errmsg\n");
					} # IF
					push @list,@entries;
	...
	foreach my $path ( @list ) {
		$status = stat($path);
		push @file_times,$status->mtime;
		push @file_sizes,$status->size;
	} # FOREACH
	if ( $options{'s'} ) {
		@indexes = sort { $file_sizes[$a] <=> $file_sizes[$b] } (0..$#list);
	} elsif ( $options{'t'} ) {
		@indexes = sort { $file_times[$a] <=> $file_times[$b] } (0..$#list);
	} else {
		@indexes = sort { $list[$a] cmp $list[$b] } (0..$#list);
	} # ELSE
	if ( $options{"R"} ) {
...
ls9.pl - display output similar to UNIX ls command

=head1 SYNOPSIS

ls9.pl [-hDdftTsScmpFiMaP] [-e exclude_pattern] [-y year] [filename [... filename]]
